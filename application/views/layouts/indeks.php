    ...
    <div class="row row-no-margin">
      ...
      <div class="col-md-4">
            ...
            <select id="i_select_category" class="selectpicker show-tick form-control" data-live-search="true">
              ...
              ?>
              <option value="<?= $row_c_header->SEO ?>"><?= $row_c_header->CATEGORY_NAME?></option>
              <?php
              $sc_header = $this->library->get_header_subcategory($row_c_header->ID); 
              foreach($sc_header as $key => $row_sc_header){ 
              ?>
              <option value="<?= $row_sc_header->SEO ?>" <?php if(strtolower($row_sc_header->SUBCATEGORY_NAME)==$data['cat']){ echo "selected"; }?>><?= ucwords(strtolower($row_sc_header->SUBCATEGORY_NAME)) ?></option>
              <?php } ?>
...
<script type="text/javascript">
  ...
  $('#i_select_category').on('change', function() {
    ...
    window.location.href = '<?= base_url()."indeks/"; ?>'+category;
  });
